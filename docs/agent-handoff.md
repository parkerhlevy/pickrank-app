...
- pull request `#26` is merged into `main` through merge commit `51f1d8f` and is live in Vercel Production. The signed-in `/contests` card shows `Open`, `Entered`, and `Edit your board`, then returns directly to the existing saved lineup. Production verification confirmed all 10 ranked players remained `Saved`, `Save board` stayed disabled without a change, no browser console errors appeared, and no production data was mutated. Post-merge Linux Chromium run `32700541806` passed.
- pull request `#27` is open from the isolated `/private/tmp/pickrank-linux-ci-hardening` worktree on branch `codex/linux-ci-hardening`, based on `origin/main` merge commit `51f1d8f`. Pull request `#26` exposed two deterministic test-contract failures: an exact `Open` locator matched both the header badge and status metric, and a full-origin assertion rejected a valid `localhost` lineup URL while expecting `127.0.0.1`. Commits `7b5ab8d` and `0f8293e` add scoped status test IDs, shared `e2eAppUrl` and `expectPagePath()` helpers, an enforced `test:e2e:contracts` source check, explicit `127.0.0.1` Next.js binding, one-worker execution, retained failure traces/screenshots, generated-report lint exclusions, and a CI runner that continues through later suites before returning a combined failure. Product rules and production data are unchanged.
- the first pull-request run `32747943616` exposed one deeper origin contract: Next.js normalized `request.url` to `localhost` in the contest progress route, which moved the full-page redirect away from the configured `127.0.0.1` origin and dropped the host-scoped auth cookie. The route now builds redirects from `getRequestOrigin()`, and the contract validator scans App Router route handlers to prevent direct `request.url` redirect origins. Linux Chromium rerun `32753664963` passed in `4m 6s`, including the entered-board return flow and all later suites.
- local verification for the CI hardening follow-up passes the expanded contract scan (`13` TypeScript files), the contract unit tests (`4` tests), `npm run typecheck`, `npm run lint`, `npm test` (`36` files, `215` tests), Playwright discovery (`27` tests), `next build --webpack`, and `git diff --check`. The focused local Chromium run reached all nine lineup cases instead of skipping after the first failure, retained failure traces, and then hit the known Codex macOS `MachPortRendezvousServer` restriction before browser creation.
...
- a fresh `git fetch --prune origin` reports `origin/main` at `51f1d8f`. The primary checkout remains on local `main` at `aceb4f4`, twelve commits behind, with unrelated preserved user work. Do not reset, stash, stage, or bundle that primary-checkout work with this slice.
...
7. Repo hygiene: `.env.example` is access-blocked and its working-tree state is unverified; resolve that boundary before a clean new slice
...
```text
Review and merge pull request `#27` after final checks remain green. Wait for the production deployment and confirm the public app remains healthy. Keep the dirty main checkout untouched. Do not change product rules or production data. After this CI reliability slice is merged, return to the fresh-address end-to-end email test as a separate auth lane.
```
...
```text
Continue PickRank using the repo as source of truth. Review pull request `#27`, which hardens Linux Chromium verification and has a passing `Browser verification` run. If the diff and final checks remain green, merge the pull request, wait for the production deployment, and confirm the public app remains healthy. Keep product behavior and production data unchanged. Keep the dirty main checkout untouched. After deployment verification, return to the fresh-address end-to-end email test as a separate auth lane.
```
