...
- the approved admin evidence dashboard is committed and pushed on `codex/admin-evidence-dashboard` in pull request `#37`. It adds append-only board revisions, stable analysis subjects, contest ruleset snapshots, admin audit events, atomic board saves, lock-time evidence, operator Overview/User Data/contest evidence/Evidence Health pages, and audited pseudonymous or reason-gated identified JSON exports. User Data now opens as an explicit All users directory with search, account and eligibility filters, evidence-health filters, sorting, totals, last activity, evidence status, and 25-user pagination. Typecheck, full lint, all `38` unit test files with `224` tests, Playwright contract validation for `14` files, the webpack production build, and `git diff --check` pass after reconciliation with current `origin/main`. The authenticated Chromium review now attaches desktop screenshots for Overview, All users, user investigation, contest investigation, and Evidence Health to every pull-request browser report. Parker previously reviewed the desktop admin mock-ups and approved the All users change, commit, push, and pull-request creation. The production migration and production deployment remain separate final approval gates. No migration, production deployment, or production data change has occurred.
