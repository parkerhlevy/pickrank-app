...
```text
Run the Linux Chromium gate against the deployed empty-board behavior. Keep the untracked Taylor Loom audit artifacts separate from the legal, provider-planning, test-hardening, cleanup, and handoff/instruction lanes in the detached primary checkout.
```
