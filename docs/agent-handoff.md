...
- this machine is currently on `main`; after a 2026-08-05 fetch, live Git shows `main` is synced with `origin/main` at `078653e Polish Build Your Board mobile parity`
- there are no uncommitted gameplay diffs; the mobile Build Your Board cleanup is now part of the pushed `main` baseline
- the latest product baseline now includes one presentation-only player-pool/board terminology slice, one gameplay-page usability cleanup across public routes and entry surfaces, and the follow-on mobile Build Your Board parity cleanup
