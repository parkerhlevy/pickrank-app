...
- `origin/main` now includes the 18+ DOB gate hotfix at `b6e7a6e Implement 18+ beta DOB gate`, cherry-picked from `79ac38d` and pushed through the clean temporary worktree `/private/tmp/pickrank-main-age-gate-18`
- the primary local `main` checkout under `/Users/parkerlevy/Documents/PickRank` still has unrelated MySportsFeeds/stat-finalization working changes; do not clean, remove, or stage those files as part of the age-gate lane
...
- the latest pushed product baseline now includes the paid-preview launch-mode guard, the admin contest removal flow, the refreshed admin removal handoff, and the production 18+ DOB gate/legal alignment; `origin/main` is the source baseline for normal delivery once the next lane is selected
...
```text
Continue PickRank using the repo as source of truth. Work only on the production 18+ DOB/profile gate smoke and read-only under-18 account audit. Start from `docs/agent-handoff.md`, `spec/product_spec.md`, `spec/features/account_profile_auth.md`, `spec/features/compliance_eligibility_responsible_play.md`, `app/auth/*`, `app/profile/*`, `app/legal/*`, `lib/auth-profile.ts`, `lib/viewer-identity.ts`, `lib/contest-entry-access.ts`, and `lib/contest-entry-confirmation.ts`. Keep Phase 1 strictly free-to-play Early Access Beta: Beta Pass, no cash value, no payouts, no cash prizes, and no paid-entry behavior. Confirm production Profile shows the `DOB / 18+ check` readiness state for an approved 18+ account and that Entry Review remains free beta. Run only read-only production checks for under-18 profiles unless Parker explicitly approves a disposable under-18 form submission or remediation. Do not mutate production data without explicit approval. Keep provider/data, Remotion/video, scoring, payment, wallet, KYC, geolocation, and public paid eligibility changes out of this lane.
```
