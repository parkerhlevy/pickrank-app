...
Current branch and production reality as of 2026-08-21:

- the optional Portless local-development pilot is documented in `docs/local-development-portless.md` with root `portless.json`; `next.config.ts` allows the named `pickrank.localhost` and worktree subdomains, and Parker verified `https://pickrank.localhost` loads successfully; direct `127.0.0.1:3000`, Playwright, CI, OAuth defaults, and production behavior remain unchanged. The pilot is intentionally set aside as uncommitted local-development dirt until Parker explicitly chooses to adopt it; it is not part of the next product slice
- a fresh `git fetch --prune origin` on 2026-08-21 verified the current baseline; local `main` contains reconciliation merge commit `561cbe4` and preserves local commits `9597a31` and `54155b3` without force-push or history rewriting, and the separate How It Works presentation commit `0877e57` is pushed to `main` and deployed to Vercel Production
...
- the current main-worktree dirt is intentional and lane-separated: the set-aside Portless pilot is in `README.md`, `next.config.ts`, `docs/local-development-portless.md`, and `portless.json`; the Cloudflare cancellation fact is recorded in this handoff-only update; and `next-env.d.ts` remains generated local noise
- fetched `main` is aligned with `origin/main`, with no committed-but-unpushed work before this handoff-only correction; the remaining uncommitted files listed above are intentional, lane-separated dirt and are not part of a product slice
