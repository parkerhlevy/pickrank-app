...
- once this handoff refresh is committed, the live `main` worktree should again be effectively clean aside from generated `next-env.d.ts` churn from local Next.js and Playwright runs
...
- the live `main` worktree on 2026-07-14 is a docs-only hygiene lane plus generated `next-env.d.ts` churn; there is no active uncommitted spec clarification draft in `spec/` right now
