...
- `docs/waitlist-workflow.md` now records the narrow deliverability-prep boundary for that follow-up, and `docs/waitlist-deliverability-audit-2026-07-22.md` records the first repo/DNS pass: public DNS shows Resend return-path evidence for `auth.pickrankgames.com` but not root `pickrankgames.com`, so the next move is to verify the active production `RESEND_FROM_EMAIL`, Resend domain SPF/DKIM/DMARC status, and reply-to strategy before running the Gmail/iCloud/Outlook/work-email test matrix; keep auth, payments, wallet, scoring, final-results, DNS, Resend settings, and Vercel environment changes out of this repo slice
...
```text
Continue PickRank using the repo as source of truth. Keep explanations business-friendly. Do a narrow waitlist email-deliverability polish pass: verify Resend sending-domain DNS status for SPF/DKIM/DMARC, review the current sender address strategy, keep the welcome email low-risk for new-domain reputation, and define a small cross-inbox test matrix for Gmail, iCloud, Outlook, and work email. Do not change the waitlist capture flow, auth, eligibility, payments, wallet-ledger, scoring, or final-results behavior in this slice.
```

Definition of done:

- Resend domain status and DNS requirements are documented from the live Resend dashboard
- Sender address/domain recommendation is recorded for production waitlist emails
- Welcome email deliverability risks are identified without changing the waitlist capture flow
- Cross-inbox test plan covers Gmail, iCloud, Outlook, and at least one work-email provider
- No auth, eligibility, payment, wallet-ledger, scoring, or final-results schema work is mixed into the deliverability follow-up
