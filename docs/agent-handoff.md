...
- the Linux CI hardening follow-up is active in the isolated `/private/tmp/pickrank-linux-ci-hardening` worktree on branch `codex/linux-ci-hardening`, based on `origin/main` merge commit `51f1d8f`. Pull request `#26` exposed two deterministic test-contract failures: an exact `Open` locator matched both the header badge and status metric, and a full-origin assertion rejected a valid `localhost` lineup URL while expecting `127.0.0.1`. The branch adds scoped status test IDs, shared `e2eAppUrl` and `expectPagePath()` helpers, an enforced `test:e2e:contracts` source check, explicit `127.0.0.1` Next.js binding, one-worker execution, retained failure traces/screenshots, generated-report lint exclusions, and a CI runner that continues through later suites before returning a combined failure. Product rules and production data are unchanged.
- local verification for the CI hardening follow-up passes `npm run test:e2e:contracts`, the new contract unit tests, `npm run typecheck`, `npm run lint`, `npm test` (`36` files, `215` tests), Playwright discovery (`27` tests), `next build --webpack`, and `git diff --check`. The focused local Chromium run reached all nine lineup cases instead of skipping after the first failure, retained failure traces, and then hit the known Codex macOS `MachPortRendezvousServer` restriction before browser creation. The required Linux browser proof remains pending a pushed pull request.
...
```text
Review the local `codex/linux-ci-hardening` diff. After Parker approves commit, push, and pull request creation, run the Linux `Browser verification` gate and confirm all Playwright suites pass with the new contracts. Keep the dirty main checkout untouched. Do not change product rules or production data. After this CI reliability slice is delivered, return to the fresh-address end-to-end email test as a separate auth lane.
```
...
```text
Continue PickRank using the repo as source of truth. Review the Linux CI hardening diff on branch `codex/linux-ci-hardening`. It removes full-origin Playwright assertions, scopes repeated status labels, enforces the test contracts before Chromium installation, pins the test server to `127.0.0.1`, keeps stateful tests on one worker, retains failure evidence, and reports all failed suites in one run. If the diff is approved, commit it, push it, open a pull request, and wait for the full Linux `Browser verification` gate. Keep product behavior and production data unchanged. Keep the dirty main checkout untouched.
```
