...
For this slice: apply and verify migration `0011_final_results_profile_read_hardening.sql` in the target Supabase environment, then smoke-test Google/magic-link return URLs and final leaderboard/results display against the deployed app without changing product behavior.
```
...
- The target Supabase project has migration `0011` applied after confirming the older broad `public can read leaderboard profiles` policy is gone
- Deployed auth redirects return to the configured PickRank origin and do not follow untrusted forwarded-host input
- Deployed public final leaderboard/results pages still show saved final rows and display handles for finalized visible contests
