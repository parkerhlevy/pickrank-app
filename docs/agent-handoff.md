...
- the current `main` worktree is aligned with `origin/main` at `aeeeee4` after `git fetch --prune origin`; there are no committed-but-unpushed commits
- active main-worktree dirt includes `.env.example`, which reintroduces obsolete SportsDataIO replay/live validation variables removed by `f3ede64`; the QA copy sweep is committed on `main` and deployed to Vercel Production, while the provider/live-validation dirt remains isolated and must not be committed with another lane unless Parker explicitly approves it
...
- the current uncommitted Contests and Contest Detail presentation patch removes the duplicate Player Pool summary metric while retaining the full player-pool list, applies sentence case to visible headings and labels, and updates focused homepage assertions; it does not change contest data, entry behavior, payments, scoring, lifecycle, admin behavior, or production Supabase state; Parker-owned removal of the old public test contest remains separate
- the current uncommitted Auth and signed-out Profile presentation patch removes repeated sign-in helper copy, uses Account settings, tightens email sign-in spacing, and replaces the Before Beta Entry block with concise information-use copy plus Beta Terms and Privacy Policy links; it does not change auth actions, eligibility capture, legal terms, entry flow, payments, wallet behavior, provider writes, or production data
- the committed How It Works presentation patch applies sentence case to the page eyebrow, beta guide tile, example link, scoring example heading, table headers, and step labels, and removes the unnecessary `Public guide` status pill without changing education content, scoring rules, navigation, or contest behavior; site-wide sentence case is now the standard for future presentation slices
- the committed site-wide copy sweep extends sentence case through Home, Contests, Contest Detail, entry flow, Build Your Board, Results, Profile, Wallet, How It Works, legal pages, and internal admin labels; it removes the remaining global uppercase transforms and preserves official product names, legal document titles, contest data, and player names; `npm run typecheck`, `npm run lint`, and `npm run test` (`34` files, `206` tests) pass; Vercel Production deployment `dpl_w4vt7ikKCLZJB6bQXJCiSTmxRHWy` is `READY` for commit `665c787a625a96500d573d599749d2b96a2ad639`, while local browser verification remains subject to the existing Codex Chromium sandbox limitation
