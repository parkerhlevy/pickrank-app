...
- a fresh `git fetch --prune origin` on 2026-08-20 verified the reconciliation baseline; local `main` contains reconciliation merge commit `561cbe4` and preserves local commits `9597a31` and `54155b3` without force-push or history rewriting, and the separate How It Works presentation commit `0877e57` is now pushed to `main` and deployed to Vercel Production
