...
- after this security/final-results hardening slice is committed, the live `main` worktree still has separate uncommitted shared-foundation UI polish work plus generated `next-env.d.ts` churn and `docs/analysis/`; keep those lanes separate and avoid mixing the separate `/private/tmp/pickrank-waitlist` worktree into `main`
