...
Current branch reality on `main` as of 2026-07-18:

- `main` is currently synced with `origin/main`; there are no committed-but-unpushed changes on this machine, and the primary worktree now includes the presentation-only Contest-as-Board implementation slice plus pre-existing generated `next-env.d.ts` churn and one untracked local analysis note under `docs/analysis/`
...
- the 2026-07-17 weekly repo-maintenance pass confirmed there is still no committed-but-unpushed work on `main`, and the only separate local worktree remains `/private/tmp/pickrank-waitlist` on `codex/waitlist-workflow`
...
- the July 15 security/final-results hardening slice is now committed on `main`; keep generated `next-env.d.ts` churn and local `docs/analysis/` notes out of unrelated release commits unless a slice explicitly needs them, and avoid mixing the separate `/private/tmp/pickrank-waitlist` worktree into `main`
...
For this slice: reconcile the live Supabase schema with the repo's final-results migration baseline before any production final-results verification. Confirm why live project `jmvzdspiobcjrewndhuf` does not have `public.entry_scoring_results` or `public.contest_player_results`, apply only the missing repo migrations that are safe for the current data state, then rerun the `0011` profile-policy verification and deployed auth/leaderboard/results smoke checks.
```
...
- The target Supabase project has the missing final-results tables and policies needed by the current repo code, without widening public profile reads or entry/table mutation rules
- Deployed auth redirects still return to the configured PickRank origin and do not follow untrusted forwarded-host input
- Deployed public leaderboard/results pages keep non-final contests placeholder-only and show saved final rows/display handles only for finalized visible contests once a final contest exists
