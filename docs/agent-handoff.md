...
Current branch and production reality as of 2026-08-19:

- remote `main` is at `fc47b2f` after the durable DOB safeguard merge; the Profile status copy is being delivered on `codex/profile-status-copy` from that authoritative baseline because direct Git transport reset during push
...
- the 2026-08-19 Profile status copy follow-up is committed on `codex/profile-status-copy` at `b5cef84a`: completed beta-entry status now uses success styling and the approved ready-to-enter copy without Beta Pass or cash-detail language, while the Profile saved notice uses purple info styling and `Your account profile is updated.`; auth, eligibility, entry, payment, wallet, scoring, provider, legal, admin, and production-data behavior are unchanged
- recorded verification for the QA copy sweep is `npm run typecheck`, `npm run lint`, and `npm run test` (`34` files, `206` tests); `git show --check 42cd89e` also passes. The promoted Vercel Production deployment is `dpl_6xMmj9HoJsn9enW4tJi6PPyLwpYZ` for commit `b5cef84a252c7841253966eed203e417479377e0`, with aliases `www.pickrankgames.com`, `pickrankgames.com`, and `pickrank-app.vercel.app`; production `/profile?status=profile-saved` and `/how-it-works` returned `200`, and Vercel reported no runtime errors in the post-deploy 15-minute window
...
- MySportsFeeds is a cleared provider option for the current B2C free-to-play beta. Its support team confirmed the described use is acceptable if PickRank does not place MySportsFeeds in competition, and confirmed internal validation and storage are acceptable for that scope. B2B use requires a separate agreement. The repo includes `npm run validate:mysportsfeeds:read-only` as an internal no-Supabase probe for auth, schedule access, game-state mapping, QB `passYards`, provider IDs, and provisional snapshot shape
...
- `COMPLETED_PENDING_REVIEW` is intentionally mapped as provisional `in_progress`, not `final`; preseason technical validation and internal validation/storage permission are complete for the current B2C free-to-play beta. Future paid-contest and B2B use remain separate commercial decisions, and the official typed-`FINAL` path remains unchanged
...
- the ZenBusiness to Cloudflare Registrar transfer completed on 2026-08-19; Cloudflare now lists `pickrankgames.com` as Active in Domain Registration with auto-renew enabled and expiration `Jun 11, 2028`; Cloudflare was already the authoritative DNS provider, so no DNS, nameserver, Vercel, Supabase, auth, or email-provider change was part of the registrar-transfer slice
- post-transfer Cloudflare DNS spot-check on 2026-08-19 showed `13` records with no recommendations, including the apex Vercel A record `76.76.21.21`, `www` CNAME to `cname.vercel-dns-0.com`, current hosted-email MX/CNAME/SPF/DMARC records, and existing Resend/Amazon SES sending records; keep Cloudflare proxying changes and Cloudflare Email Routing as separate future slices
- post-transfer Vercel and Supabase checks on 2026-08-19 passed: Vercel project `pickrank-app` still lists `www.pickrankgames.com` and `pickrankgames.com` as attached domains, the latest production deployment is `READY`, Supabase project `jmvzdspiobcjrewndhuf` (`PickRank`) has Site URL `https://www.pickrankgames.com`, and its redirect URL allow-list includes `https://www.pickrankgames.com/auth/callback`, `https://pickrankgames.com/auth/callback`, `https://pickrank-app.vercel.app/auth/callback`, `https://www.pickrankgames.com/**`, `https://pickrankgames.com/**`, `https://pickrank-app.vercel.app/**`, and an older `pickrank-app-git-codex-eligibility-95f79a-parker-levys-projects.vercel.app/auth/callback` preview URL; production auth code still redirects through `/auth/callback` on the request origin, and no production Vercel `auth`, `error`, or `fatal` logs appeared for the latest deployment during the spot-check window
...
```text
Inspect tracked `.env.example` from an allowed terminal first. If it has no intended change, restore it to the committed `main` version. If it contains intended provider configuration, keep it in an explicit, isolated provider lane. After the worktree is clean, refresh `origin/main` successfully before choosing the next product slice.
```

Definition of done:

- inspect `.env.example` from an allowed terminal and decide whether any change should be restored to committed `main` or carried as an explicitly approved provider lane
...
MySportsFeeds preseason technical validation is complete after the DET at CIN and three-game live/final read-only checks. On 2026-08-19, MySportsFeeds support confirmed the described PickRank B2C free-to-play beta use is acceptable as long as PickRank does not place MySportsFeeds in competition. Support also confirmed internal validation and storage are acceptable for that scope. B2B use requires a separate agreement. MySportsFeeds is therefore a cleared option for the current B2C beta, while future paid-contest and B2B terms remain separate decisions. Do not write Supabase rows without explicit approval, and preserve the official typed-`FINAL` path.
