...
- this machine is currently on `main`; the latest pushed baseline before the contest-detail eligibility CTA polish is `702ad25 Refresh handoff after eligibility production deploy`, and generated `next-env.d.ts` typed-route churn remains unstaged noise
...
- the 2026-07-26 contest-detail eligibility CTA polish keeps the existing server-side paid-entry block unchanged while making the first contest CTA match the real eligibility state: captured-but-`pending_review` accounts now see `Eligibility Pending Review` instead of `Enter Contest - $5`, blocked accounts see `Paid Entry Unavailable`, and both route only to Payment Review as an explanation screen with confirmation still disabled
