...
- the approved 2026-07-12 homepage hero, `How PickRank works`, and `Why PickRank` copy/icon set is live in production at `https://www.pickrankgames.com` through Vercel deployment `dpl_FbY1v8AbnZkvDVTuVKdawQKeGNJD`; the production build is `READY`, the waitlist CTA still routes to `/auth`, and the lower homepage now ends with three benefit cards using calendar, brain, and trophy icons
