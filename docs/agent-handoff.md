...
- The retired $10k-per-season provider path has been removed from code, scripts, tests, and active planning. No public provider naming or production dependency remains.
- MySportsFeeds and Rolling Insights DataFeeds are the remaining candidates. Rolling Insights is now the provisional provider front-runner for technical fit and commercial fit because its private probe returned the required NFL preseason fields, Section 12 permits internal and commercial use of DataFeeds outputs, and Josh from Rolling Insights stated in Discord on 2026-08-20 that PickRank may use the data for the described use case with no change when PickRank starts pay-to-enter contests. Its Breakaway offer is approximately `$4,200/year` for post-game data before 80%/60%/30% discounts across years 1-3, implying about `$840/$1,680/$2,940` if confirmed, plus a 30-day API-key test. MySportsFeeds remains the technical control and fallback until the vendor statement is preserved in formal written terms and the remaining operational rights are confirmed. The shared transcript renders the base as “42 for the year”; Parker confirmed `$4,200/year`, matching the published post-game reference.
- Cost comparison is provisional: MySportsFeeds account flow showed $88 CAD/month Non-Live and $158 CAD/month Live with a 10-minute delay for NFL CORE + STATS. Rolling Insights published annual tiers start at $1,200 pre-game, $4,200 post-game, and $7,200 live before the reported discount, subject to Breakaway eligibility and renewal terms.
- Confirmed facts include published product/API documentation, the repo's read-only MySportsFeeds checks, the user's Rolling Insights offer, Section 12 of the [Rolling Insights Terms of Service](https://rolling-insights.com/terms-of-services/), and Josh's August 20, 2026 Discord reply. The Discord reply directly supports PickRank's described server-side ranking, display, validation, and storage use case and says there would be no change for pay-to-enter contests. Section 12 still restricts third-party distribution/display, competitive use, and third-party application access unless authorized in writing. Preserve the Discord evidence and confirm retention, caching, attribution, redistribution, rate limits, correction behavior, and third-party application authorization in formal vendor terms.
- Rolling Insights' public API docs now add a narrower technical test path: REST access, NFL schedule/game IDs and status values, preseason examples, and QB `passing_yards`/`passing_touchdowns`. The account screen shows a current-season-only trial key that expires 30 days after creation. The private probe confirmed HTTPS access, 2026 preseason coverage, final game-state values, game/team/player IDs, and quarterback passing-yard fields. Exact entitlement, correction behavior, rate limits, historical access, and real-time trial scope still need verification.
- `scripts/rolling-insights-validation-probe.mjs` is now the private GET-only probe seam. It reads `PICKRANK_ROLLING_INSIGHTS_RSC_TOKEN`, defaults to the HTTPS REST host, redacts the token, and reports schedule, daily-schedule, live-feed, game-state, and QB passing-yard evidence without writing Supabase or snapshots.
- The Rolling probe now also accepts the vendor-compatible `RSC_TOKEN` environment variable and classifies network failures, authentication failures, cache-not-modified responses, and provider responses without printing request URLs. Its live request now sends `Cache-Control: no-cache, no-store`, `Pragma: no-cache`, and a timestamp cache-buster, matching the vendor skill guidance. Its local smoke checks pass with no token, stubbed success, stubbed network failure, and stubbed `403` responses. The guided live probe also returned HTTP 200 for season, daily, and live NFL endpoints without writing provider data.
- `scripts/validate-rolling-insights-keychain.sh` is the local scheduled-run wrapper. It retrieves the trial token from the Mac Keychain service `pickrank-rolling-insights-rsc-token`, exports it only to the child probe process, and unsets it on exit. Keychain lookup succeeded locally; the current sandbox still blocks the provider host for a live wrapper run.
- Assumptions are not launch decisions: a low first-year price does not prove renewal affordability, and a Discord statement is not the same as a signed contract amendment. The vendor statement materially improves the rights assessment, but keep all provider probes private and read-only until the remaining operational rights and technical coverage are confirmed.
- The August 17, 2026 Rolling Wave meeting adds vendor-conversation evidence: the Breakaway Accelerator described a standard post-game feed at approximately `$4,200/year`, with 80% off in year 1, 60% off in year 2, and 30% off in year 3. The program requires upfront annual payment, then converts to monthly billing after 12 months, plus active community participation through Discord and monthly speakers. The transcript also described roughly 20–40 second live latency and no historical data during the trial. Treat these terms as meeting notes until Josh Hansen confirms them in writing.
- Meeting actions are to create the 30-day trial token, test NFL preseason and MLB, send Josh feedback and the cost comparison within about one week, and continue the MySportsFeeds pricing comparison. The token now exists in the account UI. NFL preseason payload coverage is confirmed by the private probe. Exact sport/tier entitlement, correction behavior, and licensing rights remain open.
What can be done now:

- Keep `/legal/terms`, `/legal/privacy`, `/legal/beta-rules`, and `/legal/responsible-play` aligned with the free beta posture and the current 20-player pool / ranked 10-player board mechanic. The hotfix legal routes use beta-ready site copy based on Parker's supplied decisions, including the `18+` beta posture, DOB collection, Playground Sports, LLC as operator, August 9, 2026 effective date, August 11, 2026 last-updated date, Washington law, King County venue, `support@pickrankgames.com`, 30-day deletion target, and 7-day enforcement review target.
...
- Confirm the waitlist form and welcome-email copy clearly state marketing consent and unsubscribe availability before any broader outreach. The waitlist form already requires explicit marketing consent and says users can unsubscribe anytime. Parker confirmed the welcome-email unsubscribe work is complete.
- The welcome-email footer now includes why the recipient is receiving the email, no-purchase/no-entry-fee/no-prizes language, unsubscribe or Resend preference handling, Privacy Policy link, the required real postal address, and the NFL/no-endorsement disclaimer. Treat the unsubscribe task as complete and keep any later campaign deliverability work separate.
- Review Ross's revised core beta docs against the site-ready Terms and Privacy copy before final publication.
- Keep first-party DOB collection in the account/profile setup flow for the `18+` beta posture. The hotfix implementation blocks under-18 users before account use or beta entry, updates Privacy/Terms/Beta Rules/Responsible Play copy, keeps the existing profile `date_of_birth` schema hook, and updates validation/tests. Production data remediation remains approval-gated and separate from this code PR. If remediation is age-only, use `restriction_reason='under_18_age_gate'`; that is the only restriction reason that can auto-resolve when stored DOB reaches 18. Non-age account, eligibility, compliance, or responsible-play holds must remain blocked until review.
...
- Define the trademark clearance and filing plan after the name decision.
- Define the copyright registration and IP-protection plan.
- Review entity structure and timing before any restructuring or related spend.
...
- Paid-preview UI behind local or Vercel Preview mode only.
...
```text
Continue PickRank using the repo as source of truth. Complete the private provider decision lane for MySportsFeeds and Rolling Insights DataFeeds. Read `spec/features/stat_finalization.md` and `spec/features/open_questions_decision_log.md` first. Confirm written licensing for free-to-play beta, future paid contests, public result display, storage, caching, redistribution, and commercial use. Compare the Rolling Insights 80% year-one offer and 30-day API-key test with the expected MySportsFeeds package cost. If credentials become available, run only a private read-only probe. Do not write Supabase data, persist provisional snapshots, build production integration, or change scoring, payments, wallet, eligibility, auth, public results, or typed `FINAL` finalization. Update this handoff only when the provider recommendation or next implementation slice changes.
```
...
Rolling Insights is the provisional provider front-runner after a successful private read-only NFL preseason probe and a direct vendor statement supporting PickRank's described use, including future pay-to-enter contests. It remains pending formal written confirmation of the Discord statement, retention/caching/attribution/redistribution terms, third-party application authorization, and correction and operational limits. MySportsFeeds remains the technical fallback and comparison control through its existing private preseason checks. Neither provider is production-approved. Resolve the remaining licensing and technical questions before persistence or integration.
