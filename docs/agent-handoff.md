...
- local `main` contains unpushed commit `42cd89e` (`QA copy sweep`) ahead of the local `origin/main` reference at `aeeeee4`; a 2026-08-19 `git fetch --prune origin` attempt failed with `Recv failure: Connection reset by peer`, so the remote reference requires a fresh verification before push
