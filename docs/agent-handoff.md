...
- the 2026-08-19 Profile status copy follow-up is committed on `codex/profile-status-copy`: completed beta-entry status now uses success styling and the approved ready-to-enter copy without Beta Pass or cash-detail language, while the Profile saved notice uses purple info styling and `Your account profile is updated.`; auth, eligibility, entry, payment, wallet, scoring, provider, legal, admin, and production-data behavior are unchanged; this follow-up is not deployed
