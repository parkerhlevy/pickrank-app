...
```text
Complete the pull request `#38` checks, merge Education and Trust, then verify the combined production deployment. Keep production checks read-only.
```
...
```text
Continue PickRank using the repo as source of truth. Complete pull request `#38`, merge the Education and Trust lane, verify the combined Vercel Production deployment, and refresh this handoff with final commit, deployment, browser, and production evidence. Keep production checks read-only. Preserve scoring rules, eligibility rules, auth providers, Supabase configuration, and production data.
```
