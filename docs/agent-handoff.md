...
```text
Continue PickRank using the repo as source of truth. Keep explanations business-friendly. If the next priority is product proofing, use `docs/preseason-free-test-contest-runbook.md` plus `/admin/contests` to rehearse the same `$0` free/test contest loop now that the narrow operator lock exists: confirm readiness, create or reuse the free/test entry, save the lineup, lock the contest with `LOCK TEST`, enter reviewed final QB stats with typed `FINAL`, and verify final leaderboard/results show zero payout with no wallet, payment, payout, KYC, geolocation, provider automation, or public paid-entry behavior. If the design lane stays open instead, continue from `docs/analysis/external-skills-pickrank-design-audit-2026-07-29.md` with the next ranked public conversion pass across `/`, `/contests`, `/contests/[contestId]`, and `/how-it-works`, keeping the work presentation/accessibility-only.
```
