...
- this machine is currently on `main`; live Git shows `main` is synced with `origin/main` at `055dc79 Refresh admin removal handoff`
- the main worktree is clean: there are no tracked edits, untracked files, or `next-env.d.ts` generated noise in this checkout
- earlier mixed local recovery work is now separated into sibling worktrees for admin removal UX, beta age gate, beta public UI cleanup, legal comment synthesis, MySportsFeeds read-only probing, repo-lane reconciliation, and SportsDataIO parked draft work; do not restart fresh feature work in `main` until Parker chooses the next lane to integrate
- the latest pushed product baseline now includes the paid-preview launch-mode guard, the admin contest removal flow, and the refreshed admin removal handoff; `origin/main` is the clean baseline for normal delivery once the next lane is selected
- current sibling worktrees are intentional preservation lanes, not main-worktree dirt: `codex/admin-contest-removal-ux`, `codex/beta-age-gate-18`, `codex/beta-public-ui-cleanup`, `codex/legal-comment-synthesis`, `codex/provider-mysportsfeeds-read-only`, `codex/repo-lane-reconciliation`, and `codex/provider-sportsdataio-parked-draft`
- `codex/beta-public-ui-cleanup` is one commit ahead of its remote; `codex/legal-comment-synthesis`, `codex/provider-mysportsfeeds-read-only`, and `codex/provider-sportsdataio-parked-draft` are each one commit ahead of `origin/main`; keep those lanes preserved until Parker chooses what to integrate or retire
...
- Verification for `codex/beta-age-gate-18` passed after syncing onto `origin/main`: `npm run typecheck`, `npm run test` (`36` files, `211` tests), `git diff --check`, the stale age-copy search across `app`, `lib`, `spec`, `docs`, `tests`, and `db` with no matches, and focused `npx playwright test tests/e2e/homepage.spec.ts tests/e2e/lineup-builder.spec.ts --workers=1` (`15` passed after the expected sandbox `listen EPERM` rerun outside the sandbox). No production data was mutated.
...
```text
18+ DOB gate review and production verification
```

Use the beta posture as the launch source of truth: free-to-play contests, Beta Pass, no cash value, no payouts, no cash prizes, and paid contests deferred behind legal/provider/payment/withdrawal/compliance gates. The hotfix branch now carries the 18+ DOB gate, legal-route alignment, specs, tests, and handoff update. The immediate next move is review, commit, push, deploy through the established path, then run read-only production verification and a separate approval-gated production-account remediation only if under-18 accounts are found.

Next recommended slice:

```text
Continue PickRank using the repo as source of truth. Review and ship the `codex/beta-age-gate-18` hotfix only. Start from `docs/agent-handoff.md`, `spec/product_spec.md`, `spec/features/account_profile_auth.md`, `spec/features/compliance_eligibility_responsible_play.md`, `app/auth/*`, `app/profile/*`, `app/legal/*`, `lib/auth-profile.ts`, `lib/viewer-identity.ts`, `lib/contest-entry-access.ts`, and `lib/contest-entry-confirmation.ts`. Keep Phase 1 strictly free-to-play Early Access Beta: Beta Pass, no cash value, no payouts, no cash prizes, and no paid-entry behavior. Preserve first-party DOB collection. Confirm saved profile readiness is recomputed from DOB against the 18+ threshold. Do not mutate production data without explicit approval. Keep provider/data, Remotion/video, scoring, payment, wallet, KYC, geolocation, and public paid eligibility changes out of this lane.
```

Definition of done:

- Start from `docs/agent-handoff.md`, `spec/product_spec.md`, `spec/features/account_profile_auth.md`, `spec/features/compliance_eligibility_responsible_play.md`, `app/auth/*`, `app/profile/*`, `app/legal/*`, and the auth/profile gate helpers
...
```text
Continue PickRank using the repo as source of truth. Use clear, ASD-STE100-inspired technical English: short sentences, active voice, one idea per sentence, consistent terms, and short paragraphs. Keep explanations business-friendly. The current launch posture is Early Access Beta: 18+ account use, free-to-play contests, Beta Pass, no cash value, no payouts, no cash prizes, and paid contests deferred behind legal/provider/payment/withdrawal/compliance gates. The local product mechanic is now a 20-player QB pool where users still save one ranked 10-player board. Before changing behavior, read `docs/agent-handoff.md`, `spec/product_spec.md`, and the relevant `spec/features/` file for the area you are touching, plus any route, QA, provider, or marketing docs that directly govern that slice. Keep the slice narrow, avoid mixing provider/admin/auth/marketing work unless the files clearly belong together, and treat `next-env.d.ts` as generated noise unless a diff proves otherwise. Do not add payment providers, withdrawals, payouts, cash-balance or wallet-ledger movement, KYC vendor integration, geolocation enforcement, provider automation as official results, scoring changes beyond approved pool-size data, or broad contest operations unless explicitly requested. Explain results business-first: what changed, why it matters, what passed, what remains risky, and what I need to do next.
```
