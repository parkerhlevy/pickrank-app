...
- the remaining blocker is now data alignment, not infrastructure: the saved public Week 1 contest is `season = 2026`, while the Replay package is `2025REG/1`, and the saved slate matchups are 2026-specific placeholders that do not cleanly line up with the 2025 package
- live package inspection shows 14 of the 15 saved quarterbacks can be lined up to numeric SportsDataIO `PlayerID` values and package `ScoreID` values, but `Kirk Cousins` appears only as Atlanta's QB2 in the recording and `Derek Carr` does not appear as a package-matched Week 1 starter at all, so one truthful one-to-one saved-slate conversion is still blocked
...
- the 2026-07-03 live UI cleanup pass now extends the shared shell treatment across the remaining public shell routes, including `/`, `/contests`, `/contests/[contestId]`, `/payment`, `/success`, `/how-it-works`, and the saved and locked lineup states on `/contests/[contestId]/lineup`, so these screens now match the newer leaderboard, results, wallet, and profile framing for status badges, section headers, contest context cards, CTA hierarchy, and placeholder states without changing behavior
...
```text
Choose one validation lane before continuing Replay provisional work: either create one internal validation slate that truthfully matches the 2025 Week 1 Replay package, or obtain a Replay package that matches the saved 2026 public contest slate. Once season and player-pool alignment exists, rerun the provisional snapshot validation on the confirmed Replay `v3` host/query flow and decide whether the next narrow step should be an internal admin preview or a background refresh job.
```
