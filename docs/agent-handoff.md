...
Current branch and production reality as of 2026-08-28:
...
- the approved mobile board correction remains uncommitted on clean branch `codex/pre-alpha-mobile-board-corrections` in `/private/tmp/pickrank-pre-alpha-mobile-board`. It compacts the mobile sticky unsaved action, adds bottom-navigation clearance, and prevents text selection while preserving the blue drag state and accessible move controls. Local typecheck, lint, all `220` unit tests, the production build, Playwright contracts, and diff checks pass; hosted Linux Chromium still requires an approved push.
- the selected Education and Trust direction is implemented but uncommitted on clean branch `codex/education-trust-visual-proof` in `/private/tmp/pickrank-education-trust-visual-proof`. How It Works uses the real PickRank visual system for a compact `2nd -> 5th -> +3 points` rank-differential example, retains the existing detailed player example and tiebreakers unchanged, and shows an allowlisted contextual return action for known PickRank origins. The unfinished signed-in Profile form explains directly below State / jurisdiction why PickRank collects that field. No scoring rule, eligibility rule, form submission, provider, schema, or production data changed.
- Education and Trust local verification passes typecheck, lint, all `223` unit tests, the production build, Playwright contracts, and diff checks. Product Design QA is recorded in `design-qa.md` as blocked because the Codex desktop macOS sandbox denied both direct Next.js binding and Portless loopback access, so no implementation screenshot could be compared with the selected mock-up. Run the hosted Linux Chromium gate and capture desktop/mobile route evidence after an approved push and before merge.
...
```text
Complete release verification for the approved Education and Trust and mobile board workspace branches. Keep the branches separate through review. Run hosted Linux Chromium, complete the Education and Trust visual comparison, and fix any release-blocking differences before merge.
```
...
```text
Continue PickRank using the repo as source of truth. Complete release verification for `codex/education-trust-visual-proof` and `codex/pre-alpha-mobile-board-corrections`. Keep the branches separate through review. Run hosted Linux Chromium, complete visual QA, and fix any release-blocking differences before merge. Preserve scoring rules, eligibility rules, auth providers, Supabase configuration, and production data.
```
